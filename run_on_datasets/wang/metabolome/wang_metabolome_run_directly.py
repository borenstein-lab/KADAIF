...
features_path = "metabolome/wang_metabolome/mtb.tsv"
metadata_path = "wang_metabolome/metadata.tsv"


franzosa_met_data = pd.read_csv(features_path, sep = "\t", index_col = 0)
franzosa_metadata = pd.read_csv(metadata_path, sep = "\t")

ibd_series = franzosa_metadata[franzosa_metadata["Study.Group"].isin(["ESRD"])]["Sample"]
...
all_res_df.to_csv('wang/metabolome/results/new_wang_metabolome_50_%s.csv' % outlier_count )
