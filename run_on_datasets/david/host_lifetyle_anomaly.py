...
pd.DataFrame.from_dict(scores_dict.items())
    ...
    for cur_time in all_times:
        ...
        if cur_time > time_dif: 
            ...
            mod = MicrobiomeIsolationForest.MicrobiomeIsolationForest(number_of_trees = 100, max_depth = 100,min_samples_to_split=2, 
                                                                      subsample_size=74, replacement= True, weights = "equal",
                                                                 pc_method = "proportion", paral = True, verbose = True)
...
###############end running window###################
    ...
    iso_scores = iso_mod.score_samples(donor_a_data)


    iso_scores_dict = {int(donor_a_data.index[i][donor_a_data.index[i].find("Stool") + 5:]): - iso_scores[i] for i in range(iso_scores.shape[0])}
    iso_scores_df = pd.DataFrame.from_dict(iso_scores_dict.items())
    iso_scores_df.columns = columns = ["collection_day", "Anomaly_Score"]
    iso_scores_df = iso_scores_df.sort_values(by = ["collection_day"])
    iso_scores_df
    ...
    for j in range(1,8):
        ...
        iso_scores_df["rolling score"] = iso_scores_df["Anomaly_Score"].rolling(j).mean()

        sns.lineplot(data = scores_df, x = "collection_day", y = "rolling score", color = "#13293d", label = "MIFo")
        sns.lineplot(data = iso_scores_df, x = "collection_day", y = "rolling score", color = "#F3293d", label = "IF")
