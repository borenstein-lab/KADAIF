...
features_path = "franzosa/franzosa_ibd_2019_metabolome/mtb.tsv"
metadata_path = "franzosa/franzosa_ibd_2019_metabolome/metadata.tsv"


franzosa_met_data = pd.read_csv(features_path, sep = "\t", index_col = 0)
franzosa_metadata = pd.read_csv(metadata_path, sep = "\t")

ibd_series = franzosa_metadata[franzosa_metadata["Study.Group"].isin(["CD", "UC"])]["Sample"]
ibd_metab_ibd = franzosa_met_data[franzosa_met_data.index.isin(ibd_series)]
ibd_metab_healthy = franzosa_met_data[~franzosa_met_data.index.isin(ibd_series)]
...
all_res_df.to_csv('franzosa/metabolome/results/new_fran_metabolome_50_%s.csv' % outlier_count)
